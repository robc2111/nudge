...
router.post('/register', validate({ body: authSchemas.register }), async (req, res) => {
  try {
    ...
    const telegram_id = String((req.body?.telegram_id ?? DEFAULT_TELEGRAM_ID)).trim();
    ...
    const token = jwt.sign({ id: userId }, ensureJwtSecret(), { expiresIn: '7d' });
...
router.post('/login', validate({ body: authSchemas.login }), async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: user.id }, ensureJwtSecret(), { expiresIn: '7d' });
    return res.json({
      token,
