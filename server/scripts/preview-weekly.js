...
async function generateWeekly({ tone, reflections }) {
  const messages = systemPrompts.weeklyCheckins.buildChat({
    tone,
    reflections,
  });
...
(async function main() {
  try {
    ...
    if (values.list) {
      ...
      rows.forEach((r) => {
        console.log(
          `- ${r.id} | ${r.name || 'N/A'} | ${r.email || 'no-email'} | tg:${r.telegram_id || '—'} | ${r.timezone || 'Etc/UTC'}`
        );
      });
      process.exit(0);
    }
    ...
    if (!userId && !telegram && !email && !name) {
      console.error(
        'Usage: node scripts/preview-weekly.js --user <uuid or partial> | --telegram <chat_id> | --email <addr> | --name <substr> [--tone ...] [--days N] [--list]'
      );
      process.exit(1);
    }
    ...
    console.log(
      `User: ${user.id} (${user.name || 'N/A'}) | TZ: ${user.timezone || 'Etc/UTC'}`
    );
    console.log(
      `Tone: ${tone} | Reflections in last ${days}d: ${reflections.length}`
    );
    console.log('---');
